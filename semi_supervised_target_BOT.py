...
for run in range(0,10):
	...
	for num_samples in samples:
		...
		numpy_file = np.load('/raid/ADDA/BOT_3D_results/BOT_3D_run_'+str(run)+'.npy')
		...
		for i in range(1,nb_classes+1):
			...
			Xi_trn_s, Xi_val_test_s, Yi_trn_s, Yi_val_test_s = train_test_split(X_all_s[class_ind[0],:,:,:,:], Y_all_s[class_ind[0],:], train_size=64, random_state = run) # For Botswana
			print(Xi_trn_s.shape, Xi_val_test_s.shape, Yi_trn_s.shape, Yi_val_test_s.shape)


			if num_samples != 64:  # With Botswana
				
				Xi_trn_s, X_val, Yi_trn_s, Y_val = train_test_split(Xi_trn_s, Yi_trn_s, train_size=num_samples, random_state=run) # With Botswana
		...
		model.load_weights('/raid/ADDA/ForNikhil/3D_BOT_weights/3D_target_weights_seed'+str(run)+'_epoch'+str(best_epoch),by_name=True)
		# source_model = load_model('/raid/ADDA/ForNikhil/simple_frames_source_model_seed'+str(run))
		...
		np.save('/raid/ADDA/logs/BOT_SS-ADDA_20epochs/3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples)+'_train_accuracy', history.history['acc'])
		np.save('/raid/ADDA/logs/BOT_SS-ADDA_20epochs/3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples)+'_val_accuracy', history.history['val_acc'])
		np.save('/raid/ADDA/logs/BOT_SS-ADDA_20epochs/3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples)+'_train_loss', history.history['loss'])
		np.save('/raid/ADDA/logs/BOT_SS-ADDA_20epochs/3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples)+'_val_loss', history.history['val_loss'])
		...
		# model.save_weights('/raid/ADDA/logs/BOT_SS-ADDA_20epochs/3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples))
	np.save('/raid/ADDA/logs/BOT_SS-ADDA_20epochs/3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples), accuracy_each_run)
