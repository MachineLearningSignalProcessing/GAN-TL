...
for run in range(1,10):
	...
	for num_samples in samples:
		...
		for i in range(1,nb_classes+1):
			...
			Xi_trn_s, Xi_val_test_s, Yi_trn_s, Yi_val_test_s = train_test_split(X_all_s[class_ind[0],:,:,:,:], Y_all_s[class_ind[0],:], train_size=0.6, random_state = run)
		...
		model.load_weights('/raid/ADDA/UH_new/UH/UH_simple_frames_source_weights_seed'+str(run),by_name=True)
		...
		np.save('/raid/ADDA/logs/UH_SS-ADDA_20epochs_0p01/Source_3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples)+'_train_accuracy', history.history['acc'])
		np.save('/raid/ADDA/logs/UH_SS-ADDA_20epochs_0p01/Source_3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples)+'_val_accuracy', history.history['val_acc'])
		np.save('/raid/ADDA/logs/UH_SS-ADDA_20epochs_0p01/Source_3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples)+'_train_loss', history.history['loss'])
		np.save('/raid/ADDA/logs/UH_SS-ADDA_20epochs_0p01/Source_3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples)+'_val_loss', history.history['val_loss'])

		score, acc_test = model.evaluate(X_test_s, Y_test_s, batch_size= batch_size)
		print('Test score :', score)
		print('Test accuracy :', acc_test)
		accuracy_each_run.append(acc_test)
		# model.save_weights('/raid/ADDA/logs/UH_SS-ADDA_20epochs/Source_3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples))
	
	np.save('/raid/ADDA/logs/UH_SS-ADDA_20epochs_0p01/Source_3D_SS-ADDA_BOT_seed_'+str(run)+'_smpls_'+str(num_samples), accuracy_each_run)		
